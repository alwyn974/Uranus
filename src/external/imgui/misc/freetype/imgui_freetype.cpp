...
//  2017/08/16: imported from https://github.com/Vuhdo/imgui_freetype into http://www.github.com/ocornut/imgui_club, updated for latest changes in ImFontAtlas,
//  minor tweaks.
...
namespace {
    ...
    struct FontInfo {
        ...
        float LineSpacing; // The baseline-to-baseline distance. Note that it usually is larger than the sum of the ascender and descender taken as absolute
                           // values. There is also no guarantee that no glyphs extend above or below subsequent baselines when using this distance. Think of it
                           // as a value the designer of the font finds appropriate.
    ...
    struct FreeTypeFont {
        bool InitFont(FT_Library ft_library, const ImFontConfig& cfg,
                      unsigned int extra_user_flags); // Initialize from an external data buffer. Doesn't copy data, and you must ensure it stays valid up to
...
#define FT_CEIL(X) (((X + 63) & -64) / 64)
    ...
    bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg, unsigned int extra_font_builder_flags)
    {
        ...
        LoadFlags = 0;
        if ((UserFlags & ImGuiFreeTypeBuilderFlags_Bitmap) == 0) LoadFlags |= FT_LOAD_NO_BITMAP;

        if (UserFlags & ImGuiFreeTypeBuilderFlags_NoHinting) LoadFlags |= FT_LOAD_NO_HINTING;
        if (UserFlags & ImGuiFreeTypeBuilderFlags_NoAutoHint) LoadFlags |= FT_LOAD_NO_AUTOHINT;
        if (UserFlags & ImGuiFreeTypeBuilderFlags_ForceAutoHint) LoadFlags |= FT_LOAD_FORCE_AUTOHINT;
        ...
        if (UserFlags & ImGuiFreeTypeBuilderFlags_LoadColor) LoadFlags |= FT_LOAD_COLOR;
    ...
    const FT_Glyph_Metrics* FreeTypeFont::LoadGlyph(uint32_t codepoint)
    {
        ...
        if (UserFlags & ImGuiFreeTypeBuilderFlags_Bold) FT_GlyphSlot_Embolden(slot);
                ...
                if (multiply_table == NULL) {
                    ...
                    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch) {
                        for (uint32_t x = 0; x < w; x++) {
                            uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4], a = src[x * 4 + 3];
                            dst[x] = IM_COL32(
                                multiply_table[DE_MULTIPLY(r, a)], multiply_table[DE_MULTIPLY(g, a)], multiply_table[DE_MULTIPLY(b, a)], multiply_table[a]);
                        }
                    }
                }
#undef DE_MULTIPLY
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
        ...
        for (const ImWchar* src_range = src_tmp.SrcRanges; src_range[0] && src_range[1]; src_range += 2)
            for (int codepoint = src_range[0]; codepoint <= (int) src_range[1]; codepoint++) {
                ...
                uint32_t glyph_index =
                    FT_Get_Char_Index(src_tmp.Font.Face, codepoint); // It is actually in the font? (FIXME-OPT: We are not storing the glyph_index..)
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++) {
            ...
            IM_ASSERT(
                buf_bitmap_current_used_bytes + bitmap_size_in_bytes <= BITMAP_BUFFERS_CHUNK_SIZE); // We could probably allocate custom-sized buffer instead.
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++) {
            ...
            if (src_glyph.Info.IsColored) dst_glyph->Colored = tex_use_colors = true;
...
static void* FreeType_Realloc(FT_Memory /*memory*/, long cur_size, long new_size, void* block)
{
    ...
    if (block == NULL) return GImGuiFreeTypeAllocFunc((size_t) new_size, GImGuiFreeTypeAllocatorUserData);
