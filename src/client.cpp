...
UdpClient::UdpClient(std::string address, std::string port, boost::asio::io_context& ioContext) :
    _ioContext(ioContext), _resolver(ioContext), _query(boost::asio::ip::udp::v4(), address, port), _socket(ioContext), _address(address), _port(port)
{
...
{
    std::cout << "NONNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN" << std::endl;
    if (error) {
...
{
    _socket.async_send_to(boost::asio::buffer(msg, msg.size()), _remoteEndpoint,
                          boost::bind(&UdpClient::handleSend, this, boost::asio::placeholders::error));
}
